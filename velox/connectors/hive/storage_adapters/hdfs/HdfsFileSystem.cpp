...
namespace {
std::string_view stripHdfsScheme(std::string_view path) {
  if (path.find(HdfsFileSystem::kScheme) != 0) {
    return path;
  }

  const auto pathStart = path.find('/', HdfsFileSystem::kScheme.length());
...
void HdfsFileSystem::rename(
  ...
  newPath = stripHdfsScheme(newPath);
