...
TEST_F(KafkaConnectorTest, testConfig) {
  ...
  const std::shared_ptr<const KafkaConfig> kafkaConfig =
      std::make_shared<const KafkaConfig>(connectorConfig);
  ASSERT_TRUE(kafkaConfig->exists(
      connector::kafka::ConnectionConfig::kBootstrapServers));
  ASSERT_TRUE(kafkaConfig->exists(connector::kafka::ConnectionConfig::kTopic));
  ASSERT_TRUE(
      kafkaConfig->exists(connector::kafka::ConnectionConfig::kClientId));
  ASSERT_TRUE(
      kafkaConfig->exists(connector::kafka::ConnectionConfig::kGroupId));
...
TEST_F(KafkaConnectorTest, testExplicitSplitAssignsWithoutSubscribe) {
  const std::unique_ptr<DataSource> dataSource = createDataSource();
  KafkaDataSource* kafkaDataSource =
      reinterpret_cast<KafkaDataSource*>(dataSource.get());
  ASSERT_TRUE(kafkaDataSource != nullptr);
  ...
  ASSERT_EQ(tps[0].get_topic(), kafkaTopic);
  ASSERT_EQ(tps[0].get_partition(), 0);
}

TEST_F(KafkaConnectorTest, testEmptySplitAssignsPartitionsForTask) {
  const std::unique_ptr<DataSource> dataSource = createDataSource();
  KafkaDataSource* kafkaDataSource =
      reinterpret_cast<KafkaDataSource*>(dataSource.get());
  ASSERT_TRUE(kafkaDataSource != nullptr);
  kafkaDataSource->addSplit(createEmptyKafkaSplit());
  ...
  for (const auto& topicPartition : tps) {
    ASSERT_EQ(topicPartition.get_topic(), kafkaTopic);
  }
}
...
TEST_F(KafkaConnectorTest, testCreateDeserializer) {
  ...
  const std::shared_ptr<KafkaStreamJSONRecordDeserializer> jsonDeserializer =
      std::dynamic_pointer_cast<KafkaStreamJSONRecordDeserializer>(
          deserializer);
...
TEST_F(KafkaConnectorTest, testDeserializeMessages) {
  ...
  const auto& deserializer = kafkaDataSource->getDeserializer();
  std::string msg =
      "{\"event_type\":1, \"bid\": {\"auction\":1, \"bidder\":222, \"price\":1113, \"channel\":\"OTS\", \"url\":\"http://testkafka/a/b/c\", \"dateTime\":\"2025-06-18 11:22:33\", \"extra\":\"xxxx\"}}";
  VectorPtr vec1 = RowVector::createEmpty(outputType, memoryPool.get());
  vec1->resize(1);
  deserializer->deserialize(msg, 0, vec1);
  std::shared_ptr<RowVector> rowVector1 =
      std::dynamic_pointer_cast<RowVector>(vec1);
  ASSERT_TRUE(rowVector1 != nullptr);
  ASSERT_TRUE(rowVector1->size() == 1);
  ASSERT_TRUE(rowVector1->children().size() == 2);
  std::shared_ptr<FlatVector<int32_t>> flat =
      std::dynamic_pointer_cast<FlatVector<int32_t>>(rowVector1->childAt(0));
  ASSERT_TRUE(flat != nullptr);
  ASSERT_TRUE(flat->size() == 1);
  ASSERT_TRUE(flat->valueAt(0) == 1);
  std::shared_ptr<RowVector> subRow =
      std::dynamic_pointer_cast<RowVector>(rowVector1->childAt(1));
  ASSERT_TRUE(subRow != nullptr);
  ASSERT_TRUE(subRow->size() == 1);
  ASSERT_TRUE(subRow->children().size() == 7);
  std::shared_ptr<FlatVector<int64_t>> f1 =
      std::dynamic_pointer_cast<FlatVector<int64_t>>(subRow->childAt(0));
  std::shared_ptr<FlatVector<facebook::velox::StringView>> f2 =
      std::dynamic_pointer_cast<FlatVector<facebook::velox::StringView>>(
          subRow->childAt(3));
  std::shared_ptr<FlatVector<facebook::velox::Timestamp>> f3 =
      std::dynamic_pointer_cast<FlatVector<facebook::velox::Timestamp>>(
          subRow->childAt(5));
...
TEST_F(KafkaConnectorTest, testKafkaSourceNext) {
  ...
  std::string msg1 =
      "{\"event_type\":1, \"bid\": {\"auction\":1, \"bidder\":222, \"price\":1113, \"channel\":\"OTS\", \"url\":\"http://testkafka/a/b/c\", \"dateTime\":\"2025-06-18 11:22:33\", \"extra\":\"xxxx\"}}";
  std::string msg2 =
      "{\"event_type\":2, \"bid\": {\"auction\":2, \"bidder\":223, \"price\":1114, \"channel\":\"OTS-1\", \"url\":\"http://testkafka/a/b/c\", \"dateTime\":\"2025-06-18 11:22:33\", \"extra\":\"xxxx\"}}";
  std::string msg3 =
      "{\"event_type\":3, \"bid\": {\"auction\":3, \"bidder\":225, \"price\":1115, \"channel\":\"OTS-2\", \"url\":\"http://testkafka/a/b/c\", \"dateTime\":\"2025-06-18 11:22:33\", \"extra\":\"xxxx\"}}";
  ...
  std::shared_ptr<FlatVector<int32_t>> flat =
      std::dynamic_pointer_cast<FlatVector<int32_t>>(rowVector->childAt(0));
  std::shared_ptr<RowVector> subRow =
      std::dynamic_pointer_cast<RowVector>(rowVector->childAt(1));
  ASSERT_TRUE(flat != nullptr && flat->size() == 3);
  ASSERT_TRUE(subRow != nullptr && subRow->size() == 3);
  std::shared_ptr<FlatVector<int64_t>> subF1 =
      std::dynamic_pointer_cast<FlatVector<int64_t>>(subRow->childAt(0));
...
TEST_F(KafkaConnectorTest, testSnapshotStateRecordsSplitOffset) {
  const std::unique_ptr<DataSource> dataSource = createDataSource();
  KafkaDataSource* kafkaDataSource =
      reinterpret_cast<KafkaDataSource*>(dataSource.get());
  ASSERT_TRUE(kafkaDataSource != nullptr);
  ...
  const auto snapshots = kafkaDataSource->snapshotState(123);
  ASSERT_EQ(snapshots.size(), 1);
  const auto obj = folly::parseJson(snapshots[0]);
  ASSERT_EQ(obj["connector"].asString(), "kafka");
  ASSERT_EQ(obj["checkpointId"].asInt(), 123);
...
TEST_F(KafkaConnectorTest, testSnapshotStateRecordsNextOffset) {
  ...
  auto snapshots = kafkaDataSource->snapshotState(123);
  ASSERT_EQ(snapshots.size(), 1);
...
TEST_F(KafkaConnectorTest, testRestoreStateOverridesSplitOffset) {
  const std::unique_ptr<DataSource> dataSource = createDataSource();
  KafkaDataSource* kafkaDataSource =
      reinterpret_cast<KafkaDataSource*>(dataSource.get());
  ASSERT_TRUE(kafkaDataSource != nullptr);
