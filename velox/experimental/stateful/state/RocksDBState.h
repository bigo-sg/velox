...
class RocksDBState {
 public:
  RocksDBState(
      ...
      : db_(db),
        ...
        sharedKeyAndNamespaceSerializer_(
            std::make_shared<stateful::SerializedCompositeKeyBuilder<K, N>>(
                keySerializer,
                namespaceSerializer,
  ...
  std::string serializeKeyWithGroupAndNamespace(const K& key, const N& ns) {
    ...
    return sharedKeyAndNamespaceSerializer_->buildCompositeKeyNamespace(
        key, ns);
  }
  ...
  void remove(const K& key) {
    try {
      db_.Delete(
          writeOptions_,
          &columnFamily_,
          serializeKeyWithGroupAndNamespace(key, currentNamespace_));
  ...
  void put(const K& key, const N& ns, const V& value) {
    ...
    try {
      ...
      if (!status.ok()) {
        VELOX_FAIL(
            "Failed to put value into rocksdb, with key {}, namespace {}",
            key,
            ns);
      }
    } catch (const std::exception& e) {
  ...
  void remove(const K& key, const N& ns) {
    ...
    try {
      ...
      if (!status.ok()) {
        VELOX_FAIL(
            "Failed to remove from rocksdb, with key {}, namespace:{}",
            key,
            ns);
      }
    } catch (const std::exception& e) {
 ...
 protected:
  ...
  const std::shared_ptr<stateful::SerializedCompositeKeyBuilder<K, N>>
      sharedKeyAndNamespaceSerializer_;
  memory::MemoryPool* pool_;
};

template <typename K, typename N, typename V>
class RocksDBValueState : public RocksDBState<K, N, V>,
                          public ValueState<K, N, V> {
 public:
  RocksDBValueState(
      rocksdb::DB& db,
      const rocksdb::ReadOptions& readOptions,
...
template <typename K, typename N, typename V>
class RocksDBListState : public RocksDBState<K, N, ArrayVectorPtr>,
                         public stateful::ListState<K, N, V> {
 public:
  RocksDBListState(
      rocksdb::DB& db,
      ...
      memory::MemoryPool* pool)
      : RocksDBState<K, N, ArrayVectorPtr>(
            db,
            readOptions,
            writeOptions,
            columnFamily,
  ...
  void vectorUpdate(const K& key, const N& ns, const ArrayVectorPtr& vec)
      override {
    RocksDBState<K, N, ArrayVectorPtr>::put(key, ns, vec);
  }

  void vectorAdd(const K& key, const N& ns, const ArrayVectorPtr& vec)
      override {
    ArrayVectorPtr stateVector =
        RocksDBState<K, N, ArrayVectorPtr>::get(key, ns);
    if (stateVector) {
  ...
  void add(const K& key, const N& ns, const V& value) override {
    ...
    if (stateVector) {
      ...
      const auto valueSerializer =
          std::dynamic_pointer_cast<ComplexVectorSerializer<ArrayVectorPtr>>(
              RocksDBState<K, N, ArrayVectorPtr>::valueSerializer_);
      auto type = valueSerializer->getDataType();
      VELOX_CHECK(type->kind() == TypeKind::ARRAY, "Type is not an array.");
      stateVector = std::make_shared<ArrayVector>(
          this->pool_, type, nullptr, 1, BufferPtr(), BufferPtr(), 0);
...
class RocksDBMapState : public RocksDBState<K, N, MapVectorPtr>,
                        public stateful::MapState<K, N, UK, UV> {
 public:
  RocksDBMapState(
      rocksdb::DB& db,
      rocksdb::ReadOptions& readOptions,
  ...
  void put(const K& key, const N& ns, const UK& userKey, const UV& value)
    ...
    if (!mapVector) {
      const auto valueSerializer =
          std::dynamic_pointer_cast<ComplexVectorSerializer<MapVectorPtr>>(
              RocksDBState<K, N, MapVectorPtr>::valueSerializer_);
      auto type = valueSerializer->getDataType();
