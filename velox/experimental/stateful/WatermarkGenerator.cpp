  ...
  for (vector_size_t i = 0; i < timestampSize; ++i) {
    if (timestamps[i] > currentWatermark) {
      ...
      if (currentWatermark > nextWatermarkThreshold) {
        ...
        // After emit, the caller is expected to have advanced lastWatermark,
        // so recompute threshold from the new watermark.
...
int64_t WatermarkGenerator::generate(const RowVectorPtr& input) {
...
void WatermarkGenerator::close() {
  ...
  try {
    ...
    // Release ownership even if close throws to avoid pinning memory pools.
    op_.reset();
    throw;
  }
  op_.reset();
